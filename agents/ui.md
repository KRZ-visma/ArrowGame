...
- **Beta level** — Menu action loads the hand-authored demo (`js/beta-level.js`). Outside the pack: no stars, unlocks, skips, or level-index save. Win splash has no stars; primary returns to the pack level you left; retry / Restart replay the beta. Skip stays disabled while `state.beta`
