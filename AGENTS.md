...
- Levels ship as a static pack in `js/levels-data.js`; regenerate with `npm run generate-levels -- [count]` (default 100). Every baked level must pass `isSolvable` (the generator repairs filler deadlocks; the script exits if a level is still stuck). After build, `orderLevelsByComplexity` keeps the tutorial at index 0 and sorts the rest by `levelComplexity` so difficulty is non-decreasing (raw `buildLevelForIndex` order is not the shipped order — the param curve used to restart at size 8 after the hand pack)
...
| Escape / occupancy / move rules | `js/logic.js` | `canEscape`, `canEscapePath`, occupancy, `isSolvable` / `stuckArrows` (greedy clear is enough — leaving only frees cells), `levelComplexity` (size-led integer score) — **unit tests** |
| Level data & generation | `js/levels.js`, `js/level-build.js`, `js/levels-data.js` | Static pack + build helpers; `buildSolvableLevel` places multi-cell puzzle arrows (tails in center zone) then `fillEmptyCells` fills remaining **center** cells (prefer length 2, allow length 1; edges may stay empty); `repairToSolvable` then reorients leftover deadlocks (`fillEmptyCells` does not check escape, so two fillers can face each other); repair may reverse a snake (new tail can sit on an edge; at least one endpoint stays in the center); TUTORIAL is built via `buildTutorial()` for consistency; `generate-levels` runs `orderLevelsByComplexity` before writing the pack — **unit tests** |
...
- Ship a pack whose `levelComplexity` drops between adjacent levels — keep the tutorial first, then sort the rest; do not treat `buildLevelForIndex(i)` as the playable level `i`
